...
datum/reagent/honkhonkhonk/on_mob_life(mob/living/M)
	if(iscarbon(M))
		...
		if(current_cycle % 2 == 0)
			M.playsound_local(get_turf(M), 'sound/spookoween/scary_horn.ogg', 25, 1)
		if(prob(5))
			M.emote(pick("twitch","drool","moan"))
		if(prob(5))
			//var/honktoplay = pick('sound/spookoween/scary_horn.ogg','sound/spookoween/scary_horn2.ogg','sound/spookoween/scary_horn3.ogg')
			M.playsound_local(get_turf(M), 'sound/spookoween/scary_horn3.ogg', 50, 1)
		if(prob(3))
			var/phonk_message = pick("H O N K .", "CLOWN PLANET BECKONS.", "HONK !")
